...
# Lokaler Port für Cloud SQL Proxy (5432 ist bereits belegt)
PROXY_PORT="${PROXY_PORT:-55432}"

# Default-Backend-URL, kann via BACKEND_URL überschrieben werden
...
echo
echo "$(ts) 2) Backend-Dependencies (inkl. Prisma CLI aus package-lock) installieren"
echo "      -> stellt sicher, dass Prisma 5.x aus backend/package.json verwendet wird – nicht Prisma 7."
npm ci --include-workspace-root --workspace=backend
...
export DATABASE_URL="postgresql://${DB_USER}:${DB_PASS}@127.0.0.1:${PROXY_PORT}/${DB_NAME}?schema=public"
echo "$(ts) DATABASE_URL (maskiert): postgresql://${DB_USER}:***@127.0.0.1:${PROXY_PORT}/${DB_NAME}?schema=public"

echo
echo "$(ts) 2.2) Cloud SQL Proxy starten gegen ${INSTANCE_CONNECTION_NAME} auf Port ${PROXY_PORT}"
...
echo "$(ts) Warte bis Port ${PROXY_PORT} erreichbar ist..."
READY=0
...
if [ "${READY}" -ne 1 ]; then
  echo "$(ts) FEHLER: Port ${PROXY_PORT} wurde nicht rechtzeitig erreichbar. Proxy/Instanz prüfen."
  exit 1
fi

echo
echo "$(ts) 2.3) Prisma-Migration ausführen (lokale Prisma-Version aus backend/node_modules)"
cd backend

echo "$(ts) Prisma-Version (Soll: 5.x laut package-lock)"
npx prisma --version

# Wichtig: npx nutzt jetzt die lokal installierte Version (aus npm ci), nicht Prisma 7
